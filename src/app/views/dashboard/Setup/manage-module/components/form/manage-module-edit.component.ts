              ...
              <div class="input-group input-group-sm input-group-rounded">
                ...
                <div class="custom-control custom-checkbox d-flex align-items-center">
                  ...
                  <label class="custom-control-label todo-label badge-gm" for="isChecked">
                    <span
                      class="badge text-bg-{{openingForm.value.isChecked ?'success': 'danger'}} text-white">{{this.openingForm.value.isChecked ? 'ACTIVADO' : 'DESACTIVADO'}}</span>
                  </label>
                </div>
              </div>
            </div>
...
export class ManageModuleEditComponent implements OnInit {
  abcForms: any;
  public error: string = '';
  ...
  openingForm = new FormGroup({
    ...
    isChecked: new FormControl(true, [Validators.required]),
  ...
  public manageModulePatchValue(manageModule: ManageModule): void {
    manageModule.fecha_inicio = this.patchDateObject(manageModule.fecha_inicio);
    manageModule.fecha_fin = this.patchDateObject(manageModule.fecha_fin);
    // @ts-ignore
    this.openingForm.patchValue(manageModule);
  }
    ...
    if (this.openingForm.valid) {
     // @ts-ignore
      this.confirmDialogService.confirmSave().then(() => {
        this.manageModuleService.update$(this.idManage,this.openingForm.value).subscribe(response => {
