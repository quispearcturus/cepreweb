    ...
    <div class="row">
      <div class="col-md-7">
        <form class="input-group mb-3 input-gm-search" [formGroup]="filterForm">
          ...
          <button
            type="button"
            class="btn btn-primary btn-gm-search text-white"
            (click)="goFilter()"
          >
            <span class="{{ abcForms.btnSearch.icon }}"></span>
            {{ abcForms.btnSearch.label }}
          </button>
        </form>
        <div class="mb-2">
          <button
            type="button"
            (click)="goPDF()"
            class="btn btn-danger btn-gm-sm text-white"
          >
          ...
          <button
            type="button"
            (click)="goEXPORT()"
            class="btn btn-success btn-gm-sm text-white"
          >
      ...
      <div class="col-md-5 d-flex justify-content-end align-items-end mb-3">
        <button type="button" (click)="goNew()" class="btn-gm-danger">
          <span class="{{ abcForms.btnNew.icon }} lamb-icon"></span
          >{{ abcForms.btnNew.label }} Beca
        </button>
      </div>
    </div>
  `,
