  ...
  template: `
      <div class="responsive-table">
          <table class="table table-lg table-hover table-striped table-sm">
              ...
              <tr *ngFor="let v of cicles ; let i=index">
                  <th scope="row">{{i + 1}}</th>
                  <td data-title="Año">{{v.anio}}</td>
                  <td data-title="Periodo">{{v.periodo}}</td>
                  <td data-title="Fin ciclo">{{v.fe_inicio_ciclo}}</td>
                  <td data-title="fin ciclo">{{v.fe_fin_ciclo}}</td>
...
export class CicleListComponent implements OnInit {
  abcForms: any;
  @Input() cicles: Cicle[] = [];
