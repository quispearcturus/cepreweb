    ...
    <div class="row">
      ...
      <div class="col-md-5 d-flex justify-content-end align-items-end mb-3">
        <button type="button" (click)="goNew()" class="btn-gm-danger">
          <span class="{{ abcForms.btnNew.icon }} lamb-icon"></span> {{ abcForms.btnNew.label }} Ciclo
        </button>
      </div>
    </div>
  `
})
export class CicleFilterComponent implements OnInit {
  @Input() title: string = '';
