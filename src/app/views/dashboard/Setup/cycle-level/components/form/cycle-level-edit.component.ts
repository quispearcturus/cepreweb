  ...
  template: `
    ...
    <div>
      <ul class="nav nav-tabs">
        <li class="nav-item">
          <a class="nav-link active"><i class="{{ abcForms.btnNew.icon }}"></i> Edición de Nivel Ciclo</a>
        </li>
      </ul>
        ...
        <div class="form-group col-md-2 required">
          ...
          <div class="input-group input-group-sm input-group-rounded">
            <input type="text" class="form-control form-control-sm" formControlName="resolucion_directoral"
                   ...
                   placeholder="Eesolucion Directoral">
          </div>
          <app-form-validate-errors [group]=cycleForm
...
export class CycleLevelEditComponent implements OnInit {
  abcForms: any;
  public error: string = '';
  public idCycleLevel: number = 0;
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(res => {
      this.idCycleLevel = parseInt(res['idCycleLevel']);
      this.academiCycleGetById(this.idCycleLevel);
    });


  }

  academiCycleGetById(idCycleLevel: number): void {
    this.cycleLevelService.getById$(idCycleLevel).subscribe(response => {
      this.cycleLevel = response.data;
      //@ts-ignore
      this.cycleForm.patchValue(this.cycleLevel);
    });
  }
  ...
  public saveForm(): void {
    if (this.cycleForm.valid) {
      ...
      this.confirmDialogService.confirmUpdate().then(() => {
        this.cycleLevelService.update$(this.idCycleLevel, this.cycleForm.value).subscribe(response => {
