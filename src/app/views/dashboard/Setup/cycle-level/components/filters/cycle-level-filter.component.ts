...
@Component({
  selector: 'app-cycle-level-filter',
  template: `
    <div class="row">
      <div class="col-md-7">
      ...
      <div class="col-md-5 d-flex justify-content-end align-items-end mb-3">
        <button type="button" (click)="goNew()" class="btn-gm-danger">
          <span class="{{ abcForms.btnNew.icon }} lamb-icon"></span> {{ abcForms.btnNew.label }} Nivel de Ciclo
        </button>
      </div>
    </div>
  `
})
export class CycleLevelFilterComponent implements OnInit {
  @Input() title: string = '';
